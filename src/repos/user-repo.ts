...
export class UserRepository implements CrudRepository<User> {
    ...
    async getById(id: number): Promise<User> {
        ...
        try {
            client = await connectionPool.connect();
            let sql = `${this.baseQuery} order by eu.ers_user_id`;
            let rs = await client.query(sql, [id]); //rs stands for ResultSet
    ...
    async save(newUser: User): Promise<User> {
        ...
        try {
            ...
            let role_id = (await client.query('select role_id from ers_user_roles where role_name = $1', [newUser.role])).rows[0].role_id;
            ...
            newUser.role = rs.rows[0].role_name;
    ...
    async update(updatedUser: User): Promise<boolean> {
        ...
        let role_id = (await client.query('select id from ers_roles where role_name = $1', [updatedUser.role])).rows[0].role_id;
